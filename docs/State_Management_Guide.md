...
-   **주요 사용 사례 (When to use):**
    -   `vrmManager`, `pluginManager`, `actionRegistry`, **`sequenceManager`** 와 같이 애플리케이션의 핵심적인 관리자(Manager) 인스턴스를 생성하고, 모든 UI 컴포넌트가 접근할 수 있도록 제공할 때. (의존성 주입)
...
### 라. `SequenceManager`: 시퀀스의 런타임 상태 관리

-   **무엇인가?**: 모든 시퀀스의 생명주기와 런타임 상태를 중앙에서 관리하는 클래스입니다.
-   **핵심 특징**:
    -   **유효 범위**: 렌더러 프로세스 전역 (주로 `AppContext`를 통해 접근)
    -   **생명 주기**: **인-메모리(In-Memory).** 앱을 재시작하면 `electron-store`에서 활성화 목록을 읽어와 상태를 복원합니다.
    -   **데이터 타입**: 시퀀스 파일 이름(string), 활성화 상태(Set), 캐시된 시퀀스 데이터(Map) 등 시퀀스 관리에 필요한 모든 데이터를 포함합니다.
    -   **성능**: 상태 정보는 메모리에 있어 빠르지만, 시퀀스 로딩 시 파일 I/O가 발생할 수 있습니다.

-   **주요 사용 사례 (When to use):**
    -   어떤 시퀀스가 현재 활성화되어 있는지 추적하고 관리할 때.
    -   자주 사용되는 시퀀스 데이터를 메모리에 캐싱하여 성능을 최적화할 때.
    -   UI(예: `CreatorPanel`)가 현재 활성화된 시퀀스 목록을 표시하고 토글할 수 있도록 상태를 제공할 때.

-   **사용하면 안 되는 경우 (When NOT to use):**
    -   시퀀스와 관련 없는 일반적인 사용자 설정 (-> `electron-store` 사용).
    -   플러그인 간의 범용적인 상태 공유 (-> `ContextStore` 사용).

## 3. 빠른 비교표

| 특징             | `electron-store`                  | `AppContext` (React Context)      | `ContextStore`                      | `SequenceManager`                   |
| :--------------- | :-------------------------------- | :-------------------------------- | :---------------------------------- | :---------------------------------- |
| **주요 목적**    | **영속적인 사용자 설정**          | **UI 상태 및 핵심 모듈 주입**     | **플러그인/모드 간의 상태 공유**    | **시퀀스 런타임 상태 관리**         |
| **유효 범위**    | 앱 전역 (메인 + 렌더러)           | React 컴포넌트 트리 (렌더러)      | 렌더러 프로세스 전역                | 렌더러 프로세스 전역                |
| **생명 주기**    | **영속적 (디스크)**               | 인-메모리 (새로고침 시 초기화)    | 인-메모리 (새로고침 시 초기화)      | 인-메모리 (앱 시작 시 복원)         |
| **데이터 타입**  | JSON 직렬화 가능                  | 모든 타입                         | 모든 타입 (원시 타입 권장)          | 시퀀스 관련 데이터 및 상태          |
| **성능**         | 느림 (디스크 I/O)                 | 빠름 (메모리)                     | 빠름 (메모리)                       | 빠름 (메모리, 로딩 시 I/O)          |
| **데이터 소유자**| 메인 프로세스                     | React 렌더러                      | 렌더러 (전역)                       | `SequenceManager` 인스턴스          |
| **접근 방식**    | IPC 통신 (`window.electronAPI`)   | `useAppContext()` 훅              | `pluginContext.get/set`             | `useAppContext()` 훅                |
...
**질문 3: 이 데이터는 "시퀀스"의 활성화 여부나 로딩 상태와 직접적으로 관련이 있는가?**
-   **예** -> **`SequenceManager`** 의 상태로 관리하고, `AppContext`를 통해 접근하세요.
-   **아니오** -> 질문 4로 이동하세요.

**질문 4: 이 데이터는 플러그인/모드 간에 공유되거나, 플러그인의 내부 상태를 외부(다른 플러그인, UI)에 알리기 위한 것인가?**
