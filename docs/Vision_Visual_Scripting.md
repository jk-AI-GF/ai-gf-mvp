...
1.  **저장 및 불러오기 기능 복구 (직렬화)**: 현재 가장 시급한 과제입니다. `BaseNode` 모델 인스턴스를 파일로 저장(직렬화)하고, 파일로부터 다시 모델 인스턴스를 복원(역직렬화)하는 기능을 구현하여 작업 내용을 보존할 수 있도록 합니다.
2.  **데이터 흐름 구현**: 한 노드의 출력 데이터를 다른 노드의 입력으로 연결하여 사용하는, 비주얼 스크립팅의 핵심 기능을 구현합니다.
3.  **실제 이벤트 노드 추가**: '사용자 채팅 수신 시', '일정 시간 경과 시' 등 시스템 이벤트와 연동하여 자동으로 시퀀스를 시작시키는 다양한 종류의 '이벤트 노드'를 추가합니다.
