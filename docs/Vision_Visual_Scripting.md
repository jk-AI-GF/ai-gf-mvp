...
### 라. 노드의 내부 구조: 클래스 상속 모델 (구현 완료)
견고하고 확장 가능한 시스템을 구축하기 위해, 모든 노드는 공통의 `BaseNode` 클래스를 상속받는 객체 지향 모델로 구현되었습니다.

-   **`BaseNode` (추상 클래스)**: 모든 노드가 가져야 할 공통 속성(ID, 이름, 입/출력 포트 목록)과 기능(`clone()`)을 정의합니다. 가장 중요한 것은 모든 자식 클래스가 반드시 구현해야 하는 `execute()` 추상 메소드입니다.

-   **구체적인 노드 모델 (자식 클래스)**: `BaseNode`를 상속받아 각자의 특화된 `execute()` 로직을 구현합니다.
    -   `ActionNodeModel`: `execute()`가 호출되면 `PluginContext`의 `actions` API를 실행합니다.
    -   `ManualStartNodeModel`: `execute()`가 호출되면 단순히 다음 노드로 실행 흐름을 전달합니다.

-   **뷰-모델 분리**:
    -   **모델 (`core/sequence/*.ts`)**: 노드의 모든 상태와 실행 로직을 담당합니다.
    -   **뷰 (`renderer/components/SequenceEditor/*.tsx`)**: 모델의 데이터를 받아 `React Flow`에 시각적으로 렌더링하는 역할만 수행합니다.

이 구조는 코드 재사용성을 극대화하고, 새로운 노드 타입을 쉽게 추가할 수 있게 하며, '시퀀스 엔진'이 모든 노드를 `BaseNode`라는 단일 타입으로 간주하고 `execute()`만 호출하면 되는 다형성의 이점을 제공하여 엔진의 복잡도를 크게 낮춥니다.
...
### 가. 1단계: UI 및 `BaseNode` 아키텍처 구현 (완료)

-   **`BaseNode` 아키텍처 확립**: 모든 노드의 로직을 담당하는 `BaseNode` 추상 클래스와, 이를 상속받는 `ActionNodeModel`, `ManualStartNodeModel` 등 구체적인 모델 클래스 구현을 완료했습니다. 이를 통해 로직(모델)과 뷰(React 컴포넌트)를 명확하게 분리했습니다.
-   **시퀀스 에디터 UI**: `React Flow`를 기반으로 한 시퀀스 에디터 UI를 구현했습니다.
-   **동적 노드 생성**: `ActionRegistry`에 등록된 모든 액션과, `ManualStartNode` 같은 특수 노드를 사이드바에서 캔버스로 드래그 앤 드롭하여 생성할 수 있습니다.
-   **내장 파라미터 입력**: `ActionNode` 내부에 타입에 맞는(text, number, checkbox) 입력 필드를 구현하여, 다른 노드와 연결하지 않고도 파라미터 값을 직접 입력할 수 있습니다.
-   **타입 기반 포트 시스템**: 노드의 각 포트는 데이터 타입에 따라 고유한 색상을 가지며, 같은 타입의 포트끼리만 연결할 수 있습니다.

### 나. 2단계: 실행 엔진 및 저장/불러오기 (완료)

-   **시퀀스 실행 엔진**: `BaseNode` 아키텍처를 기반으로, 노드 그래프의 실행 흐름을 따라 `execute()` 메소드를 호출하는 다형적 실행 엔진의 기본 구현을 완료했습니다.
-   **수동 실행**: `ManualStartNode`와 '실행' 버튼을 통해, 에디터에서 작성한 시퀀스를 즉시 테스트 실행할 수 있습니다.
-   **저장 및 불러오기**: `BaseNode`의 `serialize()` 메서드와 IPC 통신을 통해, 사용자가 만든 시퀀스 그래프를 JSON 파일로 저장하고 다시 불러오는 기능을 구현했습니다.

### 다. 3단계: 이벤트 기반 실행 (완료)

-   **이벤트 노드 구현**: 시스템 `eventBus`에서 발생하는 이벤트를 감지하여 시퀀스를 시작하는 `EventNode`를 추가했습니다.
-   **타입-세이프 포트**: `event-definitions.ts`의 `payloadSchema`를 기반으로, 이벤트가 전달하는 데이터의 타입에 맞춰 출력 포트가 동적으로 생성됩니다. (예: `chat:newMessage` 이벤트 노드는 `role(string)`, `text(string)` 출력 포트를 가짐)
-   **이벤트 기반 엔진**: `SequenceEngine`이 시퀀스 로드 시 `EventNode`를 감지하고, 해당 `eventBus` 리스너를 자동으로 구독하도록 리팩토링했습니다.

### 라. 아키텍처 개선: SequenceManager 도입 (완료)

-   **중앙 관리자 도입**: 기존에 `App.tsx`에 혼재되어 있던 시퀀스 로딩, 활성화/비활성화, 상태 관리 로직을 분리하여 **`SequenceManager`** 클래스로 중앙화했습니다.
-   **역할 분리 (SoC)**: `App.tsx`는 이제 UI 이벤트 처리에만 집중하고, 복잡한 시퀀스 생명주기 관리는 `SequenceManager`가 전담합니다. 이로 인해 코드의 가독성과 유지보수성이 크게 향상되었습니다.
-   **안정적인 상태 관리**: `SequenceManager`가 모든 시퀀스의 활성화 상태와 캐시된 데이터를 관리함으로써, 상태 불일치로 인한 버그 발생 가능성을 줄였습니다.

### 마. 향후 계획 (Next Steps)

1.  **데이터 흐름 구현**: 현재 가장 시급하고 중요한 과제입니다. 한 노드의 출력 포트(예: 이벤트 노드의 'text' 출력)에서 다른 노드의 입력 포트(예: 액션 노드의 'message' 입력)로 실제 데이터를 전달하는, 비주얼 스크립팅의 핵심 기능을 구현합니다.
2.  **제어 흐름 노드 추가**: "Branch"(If/Else), "Delay" 등 로직 분기 및 제어를 위한 기본적인 제어 흐름 노드를 추가합니다.
3.  **사용자 경험(UX) 개선**: 노드 및 연결선 삭제, 키보드 단축키 지원, 실행 중인 노드 시각적 표시 등 에디터의 사용 편의성을 개선합니다.
