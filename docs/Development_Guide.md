...
이 프로젝트는 `PathManager`와 `ResourceManager`를 통해 리소스를 중앙에서 관리합니다.

-   **`PathManager` (`src/core/path-manager.ts`)**: **"지도 제작자"** 역할을 합니다. 개발 환경과 배포 환경의 복잡한 경로 차이를 모두 해결하고, 특정 리소스 타입(예: 'animation')에 대한 검색 경로 목록(예: `[public/animations, custom/animations]`)을 제공합니다.

-   **`ResourceManager` (`src/core/resource-manager.ts`)**: **"사서"** 역할을 합니다. `PathManager`로부터 받은 경로 목록을 사용하여, 실제 파일 시스템에서 리소스를 검색(`resolvePath`)하거나 특정 타입의 모든 리소스 목록을 취합(`listAssets`)하는 실질적인 파일 I/O 작업을 담당합니다.

-   **리소스 폴더**:
    -   **`public/`**: 앱의 기본 작동에 필수적인 **핵심 리소스**가 위치합니다.
    -   **`custom/`**: 사용자가 직접 추가하는 VRM, 애니메이션, 모드 등이 저장되는 **사용자 커스텀 리소스** 폴더입니다. `ResourceManager`는 항상 이 폴더의 리소스를 우선적으로 사용합니다.

이 구조 덕분에, 개발자는 경로 걱정 없이 `window.electronAPI.listAssets('animation')`와 같이 간단한 API 호출만으로 모든 리소스에 일관되게 접근할 수 있습니다.
